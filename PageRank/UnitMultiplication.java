...
public class UnitMultiplication {
    ...
    public static class TransitionMapper extends Mapper<Object, Text, Text, Text> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String to_string = fromTo[1].substring(1, fromTo[1].length() - 1); //remove []
            String[] tos = to_string.split(",");
            if (tos.length == 0) {
                // no outlink = link only to itself
                context.write(new Text(from), new Text(from + "=1.0"));
            }
            else {
                for (String to : tos) {
